...
export const synchroniserJoueurs = async (joueurs: Joueur[], tournoi_id: number): Promise<void> => {
...
export const upsertJoueurs = async (updatedJoueur): Promise<void> => {
  ...
  const { data, error } = await supabase.from('joueurs').upsert(updatedJoueur, { onConflict: "tournoi_id, joueur_id"}).select();
