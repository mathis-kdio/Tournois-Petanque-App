...
export const synchroniserMatchs = async (matchs: Match[], tournoi_id: number): Promise<void> => {
...
export const upsertMatchs = async (updatedMatch): Promise<void> => {
  ...
  const { data, error } = await supabase.from('matchs').upsert(updatedMatch, { onConflict: "tournoi_id, match_id"}).select();
}