...
function listeTerrains(state = initialState, action) {
  ...
  switch (action.type) {
    case 'AJOUT_TERRAIN':
      let newTerrain = {
        id: state.listeTerrains.length,
        name: 'Terrain ' + (state.listeTerrains.length + 1).toString(),
      };
      nextState = {
        ...state,
    ...
    case 'SUPPR_TERRAIN':
      if (action.value.terrainId !== undefined) {
        const liste = state.listeTerrains.filter(
          (item, index) => item.id !== action.value.terrainId,
        );
