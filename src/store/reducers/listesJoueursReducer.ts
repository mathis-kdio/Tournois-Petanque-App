...
const initialState = {
  ...
  listesSauvegarde: { avecNoms: [], sansNoms: [], avecEquipes: [] },
};
...
function listesJoueurs(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case 'SUPPR_JOUEUR': //action: 0: type d'inscription  1: id du joueur
      if (action.value[0] !== '' || action.value[1] !== '') {
        const listes = { ...state.listesJoueurs };
        listes[action.value[0]] = listes[action.value[0]].filter(
          (item, index) => item.id !== action.value[1],
        );
        //Update Ids
        for (let i = 0; i < listes[action.value[0]].length; i++) {
    ...
    case 'LOAD_SAVED_LIST': //typeInscriptionSrc: avecNoms/sansNoms/AvecEquipes/sauvegarde    //typeInscriptionDst: avecNoms/sansNoms/AvecEquipes/sauvegarde     listId: id
      if (
        action.value.typeInscriptionSrc !== undefined &&
        action.value.typeInscriptionDst !== undefined &&
        ...
        const copySeletedSavedList = JSON.parse(
          JSON.stringify(typeSavedLists[listIndex].slice(0, -1)),
        );
        listsOfPlayers[action.value.typeInscriptionDst].push(
          ...copySeletedSavedList,
        );
        //Update Ids
        for (
          let i = 0;
          i < listsOfPlayers[action.value.typeInscriptionDst].length;
          i++
        ) {
    ...
    case 'RENAME_SAVED_LIST': //typeInscription: avecNoms/sansNoms/AvecEquipes  listId: id   newName: name
      if (
        action.value.typeInscription !== undefined &&
        action.value.listId !== undefined &&
        ...
        typeSavedLists[listIndex][typeSavedLists[listIndex].length - 1].name =
          action.value.newName;
        nextState = {
          ...state,
          listesSauvegarde: savedLists,
    ...
    case 'REMOVE_SAVED_LIST': //typeInscription: avecNoms/sansNoms/AvecEquipes     listId: id
      if (action.value.listId !== undefined) {
        const savedLists = { ...state.listesSauvegarde };
        savedLists[action.value.typeInscription] = savedLists[
          action.value.typeInscription
        ].filter(
          (item, index) => item[item.length - 1].listId !== action.value.listId,
        );
        nextState = {
          ...state,
          listesSauvegarde: savedLists,
